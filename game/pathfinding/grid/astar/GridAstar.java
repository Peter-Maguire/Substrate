...
public class GridAstar implements Astar {
	...
	public GridAstar(GridLocation start, GridLocation end, GridMap map,
	...
	public GridPath getPath() {
		...
		if (!validLocation(start.getX(), start.getY())
				|| !validLocation(end.getX(), end.getY())) {
			return null;
		}

		GridLocationAstar startingLocation = new GridLocationAstar(
				start.getX(), start.getY(), start.isEnd(), 0, -1);
		sortedLocationList.add(startingLocation);
		distanceMap.set(start.getX(), start.getY(),
				startingLocation.getDoneDistance());
	...
	private GridLocation findNextLocation(int startX, int startY) {
		...
		x = startX - 1;
		if (getValue(x, y) < bestValue
				&& diagonal.isValid(diagonal.bottom_left)) {
			bestX = x;
			bestY = y;
			bestValue = distanceMap.get(x, y);
		}
		...
		x = startX + 1;
		if (getValue(x, y) < bestValue
				&& diagonal.isValid(diagonal.bottom_right)) {
			bestX = x;
			bestY = y;
			bestValue = distanceMap.get(x, y);
		}
	...
	private GridLocationAstar createLocation(int x, int y,
			GridLocationAstar previousLocation) {
		GridLocationAstar newLocation;
		boolean isEnd = end.getX() == x && end.getY() == y;
		double doneDist = previousLocation.getDoneDistance()
				+ getDist(x, y, previousLocation.getX(),
	...
	private void addLocation(int x, int y, GridLocationAstar previousLocation) {
		...
		double dist = Math.min(location.getDoneDistance(),
				distanceMap.get(x, y));
	...
	private class DiagonalMove {
		...
		private boolean isValid(int direction) {
			// Top right
			if (validLocation(x + 1, y) && validLocation(x, y + 1)
					&& direction == top_right) {
				return true;
			}
			// Bottom right
			else if (validLocation(x + 1, y) && validLocation(x, y - 1)
